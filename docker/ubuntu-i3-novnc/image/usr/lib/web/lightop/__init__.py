...
def redirectme():
    ...
    # sed
    subprocess.check_call(r"cp /etc/supervisor/conf.d/supervisord.conf /root/supervisord.conf.vorher", shell=True)
    subprocess.check_call(r"sed -i 's#^command=/usr/bin/Xvfb.*$#command=/usr/bin/Xvfb :1 -screen 0 {width}x{height}x16#' /etc/supervisor/conf.d/supervisord.conf".format(**env),
                          shell=True)
    subprocess.check_call(r"cp /etc/supervisor/conf.d/supervisord.conf /root/supervisord.conf.nachher", shell=True)
    ...
    for i in xrange(20):
        ...
        if output.strip() == "4":
            ...
            return HTML_REDIRECT
        time.sleep(2)
    abort(500, 'service is not ready. this exception is thrown by /usr/lib/web/lightop/__init__.py')
